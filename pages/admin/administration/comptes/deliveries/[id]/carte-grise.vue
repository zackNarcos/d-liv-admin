<template>
  <div>
    <DeliveryLayout>
      <div class="mb-1 mt-5 border-b border-gray-200 text-blue-800 text-2xl font-bold">
        Images de la carte grise :
      </div>
      <div class="flex mb-3 pt-2 justify-around" v-if="user?.registrationSteps?.cardGrey?.files && user?.registrationSteps?.cardGrey?.files.length > 0">
        <div class="pr-1 w-1/2 relative" v-if="user?.registrationSteps?.cardGrey?.files[0]" >
          <img ref="image" :src="user?.registrationSteps?.cardGrey?.files[0]?.url" alt="Piece d'identité" class="w-full rounded-lg border border-gray-200">
          <div class="absolute top-3 right-4 flex gap-1  rounded-full p-1">
            ...
            <Icon name="bi:link"
                  class="bg-blue-800 text-white rounded-full p-1 cursor-pointer hover:bg-blue-300 shadow-md"
                  size="30"
                  @click="copyToClipboard(user?.registrationSteps?.cardGrey?.files[0]?.url)"
            />
          </div>
        </div>
        <div class="pl-1 w-1/2 relative" v-if="user?.registrationSteps?.cardGrey?.files[1]">
          <img ref="image2" :src="user?.registrationSteps?.cardGrey?.files[1]?.url" alt="Piece d'identité" class="w-full rounded-lg border border-gray-200">
          <div class="absolute top-3 right-4 flex gap-1  rounded-full p-1">
            <Icon name="bi:eye"
                  class="bg-blue-800 text-white rounded-full p-1 cursor-pointer hover:bg-blue-300 shadow-md"
                  size="30"
            ...
            <Icon name="bi:link"
                  class="bg-blue-800 text-white rounded-full p-1 cursor-pointer hover:bg-blue-300 shadow-md"
                  size="30"
                  @click="copyToClipboard(user?.registrationSteps?.cardGrey?.files[1]?.url)"
            />
          </div>
        </div>
      </div>
...
watchEffect(() => {
  ...
  comment.value = $usersStore?.single.data?.registrationSteps?.cardGrey?.notes;
});
