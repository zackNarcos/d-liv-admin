...
onUpdated(() => {
  me.value = {
    lastName: $authedUserStore.user.lastName,
    firstName: $authedUserStore.user.firstName,
    isAdmin: $authedUserStore.user.roles.includes("admin"),
  };
});
...
const onSubmit = async () => {
  ...
  try {
    const  res = await $usersStore.updateUser(user);
    if (res.success) {
      $swal.fire({
        title: "Succès",
        text: "Compte modifier avec succès",
        icon: "success",
      });
      closeModal();
    }
