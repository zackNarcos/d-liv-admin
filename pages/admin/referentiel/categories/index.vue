<template>
  <div>
    <AdminLayout>
      ...
      <div class="my-5 text-md md:text-lg lg:text-2xl font-semibold text-gray-500">Liste des catégories</div>
      <div class="relative mb-5 bg-white border shadow-sm sm:rounded-lg">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
          ...
          <tbody v-if="$categoriesStore.all.loading || !$categoriesStore.all?.data?.length">
            <!-- Skeleton -->
            <tr v-for="i of [1, 4, 5, 6, 7, 8]" v-if="$categoriesStore.all.loading" class="border-b bg-white hover:bg-gray-100 h-12">
          ...
          <tbody v-else-if="!isEditabled">
            <tr
              v-for="data in $categoriesStore.all.data"
              :key="data._id"
              :class="{
        ...
        <div class="py-3 px-3 flex items-center justify-between">
          <div class="flex items-center gap-5">
            ...
            <div class="text-gray-500">
              ...
              {{ $categoriesStore.all?.pageCount}}
              sur {{ $categoriesStore.all?.total }}
            </div>
          </div>
          <Pagination :current-page="currentPage" :total-pages="$categoriesStore.all.pageCount" :on-page-change="handlePageChange" />
        </div>
...
<script setup>
  ...
  import AddModal from "~/pages/admin/referentiel/categories/addModal.vue";
  ...
  const toggleAllDatas = () => {
    if (selectedDatas.value.length === $categoriesStore.all.data.length) {
      selectedDatas.value = [];
      $categoriesStore.all.data.forEach((data) => {
        data.selected = false;
      });
  ...
  const editData = async (data) => {
    $categoriesStore.single.data = data;
  ...
  const deleteData = async (data) => {
    try {
      ...
      if (result.isConfirmed) {
        const res = await $categoriesStore.delete(data);
        if (res.status === 201 || res.status === 200) {
          ...
          if ($categoriesStore.all.data.length === 1 && currentPage.value > 1) {
  ...
  const published = async () => {
    ...
    try {
      ...
      // if (res.status === 200) {
      //   $categoriesStore.all.data.map((c) => {
  ...
  const orderTable = () => {
    const ids = $categoriesStore.all.data.map((c) => c._id);
    const params = {
      ids: ids,
    };
    // try {
  ...
  const fetchPaginatedData = async () => {
    ...
    try {
      await $categoriesStore.getAll(params);
    } catch (err) {
      console.error(err);
    }
  };
