<template>
  <div>
    <AdminLayout>
      ...
      <div class="my-5 text-md md:text-lg lg:text-2xl font-semibold text-gray-500">Assurances</div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
          <thead class="text-xs text-gray-700 uppercase bg-gray-100 border-t border-b">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-primary font-light">
                Nom
                <Icon name="uil:sort" class="cursor-pointer" size="16" @click="sortDatas('name')" />
              </th>
          ...
          <tbody v-if="$assurancesStore.all.loading || !$assurancesStore.all?.data?.length">
            <!-- Skeleton -->
            <tr v-for="i of [1, 4, 5, 6, 7, 8]" v-if="$assurancesStore.all.loading" class="border-b bg-white hover:bg-gray-100 h-12">
          ...
          <tbody v-else-if="!isEditabled">
            <tr
              v-for="data in $assurancesStore.all.data"
              :key="data._id"
              :class="{
        ...
        <div class="py-3 px-3 flex items-center justify-between">
          <div class="flex items-center gap-5">
            ...
            <div class="text-gray-500">
              ...
              {{ $assurancesStore.all?.pageCount}}
              sur {{ $assurancesStore.all?.total }}
            </div>
          </div>
          <Pagination :current-page="currentPage" :total-pages="$assurancesStore.all.pageCount" :on-page-change="handlePageChange" />
        </div>
...
<script setup>
  ...
  import AddModal from "~/pages/admin/referentiel/assurance/addModal.vue";

  const { $assurancesStore } = useNuxtApp();
  const $swal = swal().provide.swal;
  const title = ref("D-liv | Gestion des assurances");
  ...
  const toggleAllDatas = () => {
    if (selectedDatas.value.length === $assurancesStore.all.data.length) {
      selectedDatas.value = [];
      $assurancesStore.all.data.forEach((data) => {
        data.selected = false;
      });
  ...
  const editData = async (data) => {
    $assurancesStore.single.data = data;
  ...
  const deleteData = async (data) => {
    try {
      ...
      if (result.isConfirmed) {
        const res = await $assurancesStore.delete(data);
        if (res.status === 200 || res.status === 201) {
          ...
          if ($assurancesStore.all.data.length === 1 && currentPage.value > 1) {
  ...
  const published = async () => {
    ...
    try {
      ...
      // if (res.status === 200) {
      //   $assurancesStore.all.data.map((c) => {
  ...
  const orderTable = () => {
    const ids = $assurancesStore.all.data.map((c) => c._id);
    const params = {
      ids: ids,
    };
    // try {
  ...
  const fetchPaginatedData = async () => {
    ...
    try {
      await $assurancesStore.getAll(params);
    } catch (err) {
      console.error(err);
    }
  };
