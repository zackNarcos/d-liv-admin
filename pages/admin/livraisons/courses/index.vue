<template>
  <div>
    <AdminLayout>
      ...
      <div class="my-5 text-md md:text-lg lg:text-2xl font-semibold text-gray-500">List des Courses</div>
...
<!--            </button>-->
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
          ...
          <tbody v-if="$deliveriesStore.all.loading || !$deliveriesStore.all?.data?.length">
            <!-- Skeleton -->
            <tr v-for="i of [1, 4, 5, 6, 7, 8]" v-if="$deliveriesStore.all.loading" class="border-b bg-white hover:bg-gray-100 h-12">
          ...
          <tbody v-else>
            <tr
              v-for="data in $deliveriesStore.all.data"
              :key="data._id"
              :class="{
        ...
        <div class="py-3 px-3 flex items-center justify-between">
          <div class="flex items-center gap-5">
            ...
            <div class="text-gray-500">
              ...
              {{ $deliveriesStore.all?.pageCount}}
              sur {{ $deliveriesStore.all?.total }}
            </div>
          </div>
          <Pagination :current-page="currentPage" :total-pages="$deliveriesStore.all.pageCount" :on-page-change="handlePageChange" />
        </div>
...
<script setup>
  ...
  const toggleAllDatas = () => {
    if (selectedDatas.value.length === $deliveriesStore.all.data.length) {
      selectedDatas.value = [];
      $deliveriesStore.all.data.forEach((data) => {
        data.selected = false;
      });
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (res.status === 200 || res.status === 201) {
          ...
          if ($deliveriesStore.all.data.length === 1 && currentPage.value > 1) {
  ...
  const published = async () => {
    ...
    try {
      ...
      // if (res.status === 200) {
      //   $deliveriesStore.all.data.map((c) => {
  ...
  const orderTable = () => {
    const ids = $deliveriesStore.all.data.map((c) => c._id);
    const params = {
      ids: ids,
    };
    // try {
  ...
  const fetchPaginatedData = async () => {
    ...
    try {
      await $deliveriesStore.getAll(params);
    } catch (err) {
      console.error(err);
    }
  };
