<template>
  <div>
    <AdminLayout>
      ...
      <div class="my-5 text-md md:text-lg lg:text-2xl font-semibold text-gray-500">List des Itineraires</div>
      <div class="relative mb-5 bg-white border shadow-sm sm:rounded-lg">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
          ...
          <tr>
            ...
            <th scope="col" class="px-6 py-3 text-primary font-light">
              Source
              <Icon name="uil:sort" class="cursor-pointer" size="16" @click="sortDatas('source')" />
            </th>
            <th v-if="isEditabled" scope="col" class="px-6 py-3 text-primary font-light">Poids</th>
            <th scope="col" class="px-6 py-3 text-primary font-light">
              <div class="flex items-center">
                Destination
                <Icon name="uil:sort" class="cursor-pointer" size="16" @click="sortDatas('destination')" />
              </div>
            </th>
            <th scope="col" class="px-6 py-3 text-primary font-light">
              <div class="flex items-center">
                Date
                <Icon name="uil:sort" class="cursor-pointer" size="16" @click="sortDatas('description')" />
              </div>
            </th>
            <th scope="col" class="px-6 py-3 text-primary font-light">
              <div class="flex items-center">
          ...
          <tbody v-if="$deliveriesStore.itineraires.loading || !$deliveriesStore.itineraires?.data?.length">
          <!-- Skeleton -->
          <tr v-for="i of [1, 4, 5, 6, 7, 8]" v-if="$deliveriesStore.itineraires.loading" class="border-b bg-white hover:bg-gray-100 h-12">
          ...
          <tr v-else class="border-b bg-white hover:bg-gray-100">
            <td colspan="8">
              <p class="text-center py-8 text-gray-700">Pas de données disponible</p>
            </td>
          </tr>
          </tbody>
          ...
          <tbody v-else>
          <tr
              v-for="data in $deliveriesStore.itineraires.data"
              :key="data._id"
              :class="{
        ...
        <div class="py-3 px-3 flex items-center justify-between">
          <div class="flex items-center gap-5">
            ...
            <div class="text-gray-500">
              ...
              {{ $deliveriesStore.itineraires?.pageCount}}
              sur {{ $deliveriesStore.itineraires?.total }}
            </div>
          </div>
          <Pagination :current-page="currentPage" :total-pages="$deliveriesStore.itineraires.pageCount" :on-page-change="handlePageChange" />
        </div>
...
const { $deliveriesStore } = useNuxtApp();
const $swal = swal().provide.swal;
const title = ref("D-liv | Gestion des courses");
...
const toggleAllDatas = () => {
  if (selectedDatas.value.length === $deliveriesStore.itineraires.data.length) {
    selectedDatas.value = [];
    $deliveriesStore.itineraires.data.forEach((data) => {
      data.selected = false;
    });
...
const editData = async (data) => {
  $deliveriesStore.single.data = data;
  datasModel.value = data;
  showAddModal.value = true;
};
...
const deleteData = async (data) => {
  try {
    ...
    if (result.isConfirmed) {
      const res = await $deliveriesStore.delete(data);
      if (res.status === 200 || res.status === 201) {
        await $swal.fire({
          title: "Asurance supprimé avec succès",
          icon: "success",
          confirmButtonColor: "#15803d",
        });

        if ($deliveriesStore.itineraires.data.length === 1 && currentPage.value > 1) {
...
const published = async () => {
  ...
  try {
    ...
    // if (res.status === 200) {
    //   $deliveriesStore.itineraires.data.map((c) => {
...
const orderTable = () => {
  const ids = $deliveriesStore.itineraires.data.map((c) => c._id);
  const params = {
    ids: ids,
  };
  // try {
...
const fetchPaginatedData = async () => {
  ...
  try {
    await $deliveriesStore.getItineraires(params);
  } catch (err) {
    console.error(err);
  }
};
